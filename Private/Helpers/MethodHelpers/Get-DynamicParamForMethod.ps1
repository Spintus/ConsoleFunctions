        ...
        foreach ($overload in $overloadMethods)
        {
            ...
            foreach ($parameter in $overload.Params.Keys)
            {
                if (-not $paramRuntimeCollection.ContainsKey($parameter))
                {
                    ...
                    if ($Overload.Params[$parameter] -is [System.Management.Automation.PSReference])
                    {
                        $newParam = New-PowerObject -TypeName System.Management.Automation.RuntimeDefinedParameter -name $parameter -parameterType ($Overload.Params[$parameter].GetType()) -attributes @($attribute)
                    }
                    else
                    {
